...
const longestString = (arr) => {
  ...
  let longest = sorted[sorted.length - 1];
  console.log(longest);
...
const firstLetters = (arr) => {
  let new_arr = arr.map((string) => string.slice(0, 1));
  return new_arr;
};
...
const standardizePhoneNumbers = (arr) => {
  return arr.map(
    (element) => element.slice(1, 4) + element.slice(6, 9) + element.slice(10)
  );
};
...
const onlyOddChars = (str) => {
  ...
  str_arr.forEach((element, index) => {
    if (index % 2) new_str.push(element);
  });
  return new_str.join("");
};
...
const allHappy = (arr) => {
  ...
  return matches.length === arr.length ? true : false;
};
