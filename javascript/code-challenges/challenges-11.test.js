...
const count = (target, input) => {
  let flat = input.flat();
  console.log(flat);
  let total = flat.reduce((a, b) => {
    if (b === target) return a + 1;
    else return a;
  },
  0);
  return total;
...
const divisibleByFiveTwoToThePower = (input) => {
  let powered = input.map(arr => {
    if (typeof arr[0] !== 'number') return [];
    else {
      return arr.filter(num => num % 5 === 0).map(num => 2 ** num);
      }
    });
  return powered;
};
...
let findMaleAndFemale = (data) => {
  let names = data.filter(obj => obj.gender === 'male' || obj.gender === 'female').map(obj=> obj.name).reduce((a,b) => a + ' and ' + b, '');
