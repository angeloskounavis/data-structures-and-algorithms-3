...
const forEachTwoToThe = (arr) => {
    let new_arr = [];
    arr.forEach(value => new_arr.push(2 ** value));
    return new_arr;
};
...
const charCode = (arr) => {
  let new_arr = arr.map(x => x.charCodeAt());
  return new_arr;
};
...
const extractAbilities = (arr) => {
   let new_arr = arr.map((x) => x.ability.name);
    return new_arr;
};
...
const extractStats = (arr) => {
   let new_arr = arr.map((x) => ({name: x.stat.name, total: x.effort + x.baseStat}));
    return new_arr;
};
